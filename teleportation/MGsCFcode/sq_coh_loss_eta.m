function [F] = sq_coh_loss_eta(T, l, t, e, operation, g, eta, s, sigma)
warning('off')
...
switch operation
    case 'epr'
        ...
        F=...
            ...
            (exp(1).^((1/2).*(((-1)+l.^2).^(-1).*r.^2.*(1+l.^2+(-4).*eta.*g.* ...
  l.*t.^(1/2)+g.^2.*(1+e.*eta.^2.*((-1)+l.^2).*((-1)+t)+l.^2.*((-1)+ ...
  2.*eta.^2.*t)))+(-1).*abs(exp(1).^((sqrt(-1)*(-1)).*theta).*r.*( ...
  exp(1).^((sqrt(-1)*2).*theta).*cosh(s)+sinh(s))).^2+(-1).*abs(( ...
  -1).*exp(1).^((sqrt(-1)*(-1)).*theta).*eta.*g.*r.*(exp(1).^((sqrt( ...
  -1)*2).*theta).*cosh(s)+sinh(s))).^2)).*pi.^(-1).*r)...
            ,0,inf,0,2*pi,0,3*sigma);
    case 'ps'
        F=...
            integral3(@(r,theta,ra)...
            ra.*exp(-ra.^2./sigma).*2./sigma.*...
            (exp(1).^((1/2).*(((-1)+eta.^2).*g.^2.*r.^2+(-1).*exp(1).^((sqrt( ...
  -1)*(-1)).*theta).*r.*ra+exp(1).^(sqrt(-1).*theta).*r.*ra+exp(1) ...
  .^((sqrt(-1)*(-1)).*theta).*eta.*g.*r.*ra+(-1).*exp(1).^(sqrt(-1) ...
  .*theta).*eta.*g.*r.*ra+r.^2.*((-2)+l.^2.*((-2)+((-2)+e).*t.*((-1) ...
  +T))+e.*t.*((-1)+T)).*(2+l.^2.*((-2)+((-2)+e).*t.*((-1)+T))+e.*(t+ ...
  (-1).*t.*T)).^(-1)+8.*eta.*g.*l.*r.^2.*t.^(1/2).*T.^(1/2).*(2+ ...
  l.^2.*((-2)+((-2)+e).*t.*((-1)+T))+e.*(t+(-1).*t.*T)).^(-1)+ ...
  eta.^2.*g.^2.*r.^2.*(2+l.^2.*((-2)+((-2)+e).*t.*((-1)+T))+e.*(t+( ...
  -1).*t.*T)).^(-1).*((-2)+(-1).*e.*t.*(1+T)+l.^2.*(2+((-2)+e).*t.*( ...
  1+T)))+(-1).*abs((exp(1).^(sqrt(-1).*theta).*r+ra).*cosh(s)+(exp( ...
  1).^((sqrt(-1)*(-1)).*theta).*r+ra).*sinh(s)).^2+(-1).*abs(((-1) ...
  .*exp(1).^(sqrt(-1).*theta).*eta.*g.*r+ra).*cosh(s)+((-1).*exp(1) ...
  .^((sqrt(-1)*(-1)).*theta).*eta.*g.*r+ra).*sinh(s)).^2)).*((-2).* ...
  l.^2+e.*((-1)+l.^2)).^(-1).*pi.^(-1).*r.*(2+l.^2.*((-2)+((-2)+e).* ...
  t.*((-1)+T))+e.*(t+(-1).*t.*T)).^(-1).*(e.^2.*((-1)+l.^2).^2.*t.*( ...
  (-1)+T+eta.^2.*g.^2.*r.^2.*T)+4.*l.^2.*((-1)+r.^2+(-2).*eta.*g.* ...
  l.*r.^2.*t.^(1/2).*T.^(1/2)+l.^2.*(1+t.*((-1)+T+eta.^2.*g.^2.* ...
  ...
  1).^((sqrt(-1)*(-1)).*theta).*r+ra).*sinh(s)).^2+(-1).*abs(((-1) ...
  .*exp(1).^(sqrt(-1).*theta).*eta.*g.*r+ra).*cosh(s)+((-1).*exp(1) ...
  .^((sqrt(-1)*(-1)).*theta).*eta.*g.*r+ra).*sinh(s)).^2)).*pi.^(-1) ...
  .*r.*((-2)+(-1).*e.*t+l.^2.*(2+((-2)+e).*t)).^(-1).*(2+l.^2.*((-2) ...
  +((-2)+e).*t.*((-1)+T))+e.*(t+(-1).*t.*T)).^(-1).*(l.^4.*(2+((-2)+ ...
  e).*t).*((-2)+eta.^2.*g.^2.*r.^2.*(2+((-2)+e).*t)+((-2)+e).*t.*(( ...
  -1)+T))+(2+e.*t).*((-2)+eta.^2.*g.^2.*r.^2.*(2+e.*t)+e.*t.*((-1)+ ...
  T))+4.*eta.*g.*l.^3.*r.^2.*t.^(1/2).*(2+((-2)+e).*t).*T.^(1/2)+( ...
  ...
  .*exp(1).^(sqrt(-1).*theta).*eta.*g.*r+ra).*cosh(s)+((-1).*exp(1) ...
  .^((sqrt(-1)*(-1)).*theta).*eta.*g.*r+ra).*sinh(s)).^2)).*pi.^(-1) ...
  .*r.*(2+l.^2.*((-2)+((-2)+e).*t.*((-1)+T))+e.*(t+(-1).*t.*T)).^( ...
  -2).*(e.^2.*((-1)+l.^2).^2.*t.^2.*((-1)+T).^2+(-2).*e.*((-1)+l.^2) ...
  .*t.*(1+l.^2.*((-1)+2.*t)).*((-1)+T).^2+4.*(l.^2.*(t.*((-1)+T).^2+ ...
  (-2).*T)+T+l.^4.*((-1).*t.*((-1)+T).^2+t.^2.*((-1)+T).^2+T))).^( ...
  ...
  -1)*(-1)).*theta).*r.*ra+exp(1).^(sqrt(-1).*theta).*r.*ra+exp(1) ...
  .^((sqrt(-1)*(-1)).*theta).*eta.*g.*r.*ra+(-1).*exp(1).^(sqrt(-1) ...
  .*theta).*eta.*g.*r.*ra+8.*eta.*g.*l.*r.^2.*t.^(1/2).*T.*(2+e.*(t+ ...
  (-1).*t.*T.^2)+l.^2.*((-2)+((-2)+e).*t.*((-1)+T.^2))).^(-1)+r.^2.* ...
  ((-2)+e.*t.*((-1)+T.^2)+l.^2.*((-2)+((-2)+e).*t.*((-1)+T.^2))).*( ...
  2+e.*(t+(-1).*t.*T.^2)+l.^2.*((-2)+((-2)+e).*t.*((-1)+T.^2))).^( ...
  -1)+eta.^2.*g.^2.*r.^2.*(2+e.*(t+(-1).*t.*T.^2)+l.^2.*((-2)+((-2)+ ...
  e).*t.*((-1)+T.^2))).^(-1).*((-2)+(-1).*e.*t.*(1+T.^2)+l.^2.*(2+(( ...
  -2)+e).*t.*(1+T.^2)))+(-1).*abs((exp(1).^(sqrt(-1).*theta).*r+ra) ...
  .*cosh(s)+(exp(1).^((sqrt(-1)*(-1)).*theta).*r+ra).*sinh(s)).^2+( ...
  -1).*abs(((-1).*exp(1).^(sqrt(-1).*theta).*eta.*g.*r+ra).*cosh(s) ...
  +((-1).*exp(1).^((sqrt(-1)*(-1)).*theta).*eta.*g.*r+ra).*sinh(s)) ...
  .^2)).*pi.^(-1).*r.*((-2)+(-1).*e.*t+l.^2.*(2+((-2)+e).*t)).^(-1) ...
  .*(2+e.*(t+(-1).*t.*T.^2)+l.^2.*((-2)+((-2)+e).*t.*((-1)+T.^2))) ...
  .^(-2).*((-2)+(-1).*e.*t.*(1+T.^2)+l.^2.*(2+((-2)+e).*t.*(1+T.^2)) ...
  ).^(-1).*(16+(-8).*e.*t.*((-4)+(1+3.*eta.^2.*g.^2.*r.^2).*T.^2)+ ...
  4.*e.^2.*t.^2.*(6+(-3).*(1+3.*eta.^2.*g.^2.*r.^2).*T.^2+((-1)+2.* ...
  eta.^2.*g.^2.*r.^2+eta.^4.*g.^4.*r.^4).*T.^4)+e.^4.*t.^4.*(1+(-1) ...
  .*(1+3.*eta.^2.*g.^2.*r.^2).*T.^2+((-1)+2.*eta.^2.*g.^2.*r.^2+ ...
  eta.^4.*g.^4.*r.^4).*T.^4+(1+eta.^2.*g.^2.*r.^2).*T.^6)+2.*e.^3.* ...
  t.^3.*(4+(-3).*(1+3.*eta.^2.*g.^2.*r.^2).*T.^2+2.*((-1)+2.* ...
  ...
  -1).*abs(((-1).*exp(1).^(sqrt(-1).*theta).*eta.*g.*r+ra).*cosh(s) ...
  +((-1).*exp(1).^((sqrt(-1)*(-1)).*theta).*eta.*g.*r+ra).*sinh(s)) ...
  .^2)).*((-2).*l.^2+e.*((-1)+l.^2)).^(-1).*pi.^(-1).*r.*(2+e.*(t+( ...
  -1).*t.*T.^2)+l.^2.*((-2)+((-2)+e).*t.*((-1)+T.^2))).^(-2).*((-2)+ ...
  (-1).*e.*t.*(1+T.^2)+l.^2.*(2+((-2)+e).*t.*(1+T.^2))).^(-1).*( ...
