...
for sig_ind in range(len(sigmaT)):
    f_tmsv_i = []
    f_qs_t_i = []
    f_qs_r_i = []
    
    for it in T:
        f_tmsv_i += [tmsv.opt_fidelity_alphabet_gopt(it, eps, eta, sigmaT[sig_ind])]
        f_qs_t_i += [qs_t.opt_avg_fidelity(it, eps, eta, sigmaT[sig_ind])]
