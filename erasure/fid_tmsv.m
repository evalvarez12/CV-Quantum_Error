...
switch modes
    case '1'
        ...
        F=...
            ...
            theta).*((sqrt(-1)*8).*ra.*(1+gp)+r.*(7+V+gp.^2.*(3+5.*V+(-4).*(( ...
    ...
    case '2'
        F=...
            ...
            sin(theta).*((sqrt(-1)*(-8)).*ra.*((-1)+gp)+r.*(7+V+gp.*((-2)+2.* ...
            V+(-4).*((-1)+V.^2).^(1/2))+gp.^2.*(3+5.*V+(-4).*((-1)+V.^2).^( ...
            1/2))).*sin(theta)))).*pi.^(-1).*r)...
            ,0,inf,0,2*pi,0,3*sigma);
    ...
    case '12'
        F=...
            integral3(@(r,theta,ra)...
            ra.*exp(-ra.^2./sigma).*2./sigma.*...
            (exp(1).^((1/4).*r.*((-1).*r.*(4+gp.^2.*(1+V)+gx.^2.*(1+V))+(gp.^2+ ...
    ...
    case '13'
        F=...
            ...
            cos(theta).^2+sin(theta).*((sqrt(-1)*8).*ra.*(1+gp)+r.*(7+V+(-2).* ...
            gp.*(1+V)+gp.^2.*(3+V)).*sin(theta)))).*pi.^(-1).*r)...
            ,0,inf,0,2*pi,0,3*sigma);
    ...
    case '23'
        F=...
            ...
            gp.*(7+V)+gx.*(7+V)).*cos(2.*theta)+(sqrt(-1)*16).*ra.*((-1)+gp).* ...
            sin(theta))).*pi.^(-1).*r)...
            ,0,inf,0,2*pi,0,3*sigma);
